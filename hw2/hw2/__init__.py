    ...
    def handClose(self):
        if self.hand['closed?']:
            ...
            for (name,location) in self.blocks.iteritems():
               if not location == self.goal[name]: 
    ...
    def allowedActions(self):
        def alreadyThere(coord):
            for (block_name, block_coord) in self.blocks.iteritems():
                if (block_coord == coord) and (block_coord == self.goal[block_name])and(self.hand['held']==block_name):
                    return True
            return False
...
def graphsearch(queue,queue_modification,timeout,visualize=True):
...
def breadthFirst(queue,visited):
    ...
    key1 = first[0]
    nowstate = first[1]
    if not nowstate.isGoal():
       move = nowstate.allowedActions()
      # print move
       for i in move:
          state_copy = copy.deepcopy(nowstate)
         # print state_copy.hand
          state_copy.do(i)
          if not duplicateWorld(state_copy,visited):
               queue.insert(0,(key1,state_copy))
              # print state_copy.hand
               return[queue,queue[0][1]]
       remove = queue.pop(-1)
       return [queue,nowstate]
    else:
       return[queue,nowstate] 
...
def depthFirst(queue,visited):
    ...
    if not nowstate.isGoal():
       ...
       for i in move:
          ...
          state_copy.do(i)
          if not duplicateWorld(state_copy,visited):
               queue.insert(0,(key2,state_copy))
              # print state_copy.hand
               return[queue,queue[0][1]]
       remove = queue.pop(0)
       return [queue,nowstate]
...
def bestFirst(queue,visited,heuristic):
    ...
    if not nowstate.isGoal():
       ...
       #print move
       state_copy = copy.deepcopy(nowstate)
       chosemove=heuristic(state_copy,move,visited)
       if chosemove ==None:
            remove =queue.pop(0)
            return [queue,state_copy]
       else:      
       # print state_copy.hand
       
       # print chosemove
        state_copy.do(chosemove)
      #  if not duplicateWorld(state_copy,visited):
        queue.insert(0,(key3,state_copy))
...
def get_depth(self,state):
    now_state=state.hand['location']
    depth=0
    if state.hand['location']!=(0,0,0):
      now_state.append(state.hand['location'])
      depth+=1
    return depth
...
def hs1(self,move,visited):
    cost0=[]
    cost1=[]
    cost2=[]
    cost3=[]
    for i in move:
       statenow=copy.deepcopy(self)
       statenow.do(i)
       if not duplicateWorld(statenow,visited):
        for (block_name, block_coord) in statenow.blocks.iteritems():
            if(statenow.hand['held']==block_name)and(statenow.hand['location']==statenow.goal[block_name]):
               tp3=statenow.hand['location']
               tp4=statenow.goal[block_name]
              # distance = abs(tp3[0]-tp4[0])+abs(tp3[1]-tp4[1])+abs(tp3[2]-tp4[2])
               #temp = [i,distance]
               cost0.append((i,0))
            if(statenow.hand['held']==block_name)and(statenow.hand['location']!=statenow.goal[block_name]):
               tp3=statenow.hand['location']
       ...
       if not duplicateWorld(statenow,visited):
        for (block_name, block_coord) in statenow.blocks.iteritems():
            ...
            if(statenow.hand['held']==block_name)and(statenow.hand['location']!=statenow.goal[block_name]):
               tp3=statenow.hand['location']
               tp4=statenow.goal[block_name]
               distance =math.sqrt((tp3[0]-tp4[0])*(tp3[0]-tp4[0])+(tp3[1]-tp4[1])*(tp3[1]-tp4[1])+(tp3[2]-tp4[2])*(tp3[2]-tp4[2]))
               #temp = [i,distance]
               cost1.append((i,distance))
            else:
                # distance = abs(tp1[0]-tp2[0])+abs(tp1[1]-tp2[1])+abs(tp1[2]-tp2[2])
                #temp = [i,distance]
                 cost2.append((i,0))
    ...
    for i in move:
       ...
       if not duplicateWorld(statenow,visited):
        for (block_name, block_coord) in statenow.blocks.iteritems():
            ...
            if(statenow.hand['held']==None)and(statenow.hand['location']==block_coord):
                 tp1=statenow.hand['location']
                 tp2=block_coord
                # distance = abs(tp1[0]-tp2[0])+abs(tp1[1]-tp2[1])+abs(tp1[2]-tp2[2])
               #temp = [i,distance]
                 cost2.append((i,0))
...
hs = {hs1,hs2,hs3} #this is a set, put your heuristic functions in here.
